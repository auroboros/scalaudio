...
import scalaudio.core.types.{AudioDuration, Frame, MultichannelAudio}
...
case class SoundSample(wavetable: MultichannelAudio, samplingFreq: Double) extends WavetableType {
  ...
  def interpolatedFrame(position: Double): Frame = wavetable.indices.toArray map { c =>
    interpolatedSample(c, position)
  }
